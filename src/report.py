...
def create_report(donors_info):
    """Make a list for each column from dict data then populate row data into a table."""
    ...
    average_donation = [str(int(sum(a) / len(a))) for a in donation]
    for row in zip(donor_name, total_donation, num_of_donation, average_donation):
        print(', '.join(row))
        donor_list.append(', '.join(row))
    return sorted(donor_list)